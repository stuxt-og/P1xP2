...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCoreDispSurface::GeneratePointStartIndexFromMappingAxes( Vector const &p_sAxis, Vector const &p_tAxis )
{
    ...
    //
	float minValue = DotProduct( p_tAxis, m_Points[0] );
    ...
    {
		float value = DotProduct( p_tAxis, m_Points[i] );
    ...
    //
	minValue = DotProduct( p_sAxis, m_Points[indices[0]] );
    offsetIndex = indices[0];
    
    for( i = 1; i < numIndices; i++ )
    {
		float value = DotProduct( p_sAxis, m_Points[indices[i]] );
        if( ( value < minValue ) )
        {
            minValue = value;
