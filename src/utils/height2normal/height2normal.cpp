				   ...
				   float bumpScale )
{
	...
	for( frameID = 0; frameID < numFrames; frameID++ )
	{
		...
		*pcUnderscore = NULL;
		...
		float sourceGamma;
		CUtlBuffer l_buf;
		if ( !g_pFullFileSystem->ReadFile( heightTGAFileName, NULL, l_buf ) )
		{
			fprintf( stderr, "%s not found\n", heightTGAFileName );
			return;
		}

		if ( !TGALoader::GetInfo( l_buf, &width, &height, &imageFormat, &sourceGamma ) )
		{
		...
		l_buf.SeekGet( CUtlBuffer::SEEK_HEAD, 0 );
		TGALoader::Load( pImageIA88, l_buf, width, height, IMAGE_FORMAT_IA88, sourceGamma, false );
