...
void PerforceFileExplorer::PopulateFileList()
{
	...
	{
		const char *l_pFileName = p4->String( fileList[i].m_sLocalFile );
		if ( !l_pFileName[0] )
			continue;

		int nItemID = m_pFileList->FindFile(l_pFileName);
		...
		if ( nItemID == m_pFileList->InvalidItemID() )
		{
			...
			bFileExists = false;
			nItemID = m_pFileList->AddFile( l_pFileName, false, fileList[i].m_bDir );
		}
