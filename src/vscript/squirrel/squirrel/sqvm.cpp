    ...
    //
    {
        for(;;)
        {
            ...
            case _OP_CALL: {
                    ...
                    case OT_USERDATA:
                    case OT_INSTANCE:{
                        SQObjectPtr l_closure;
                        if(_delegable(clo)->_delegate && _delegable(clo)->GetMetaMethod(this,MT_CALL, l_closure)) {
                            Push(clo);
                            for (SQInteger i = 0; i < arg3; i++) Push(STK(arg2 + i));
                            if(!CallMetaMethod(l_closure, MT_CALL, arg3+1, clo)) SQ_THROW();
                            if(sarg0 != -1) {
                                STK(arg0) = clo;
                            }
                            break;
...
//      SQInteger n = 0;
        ...
        while( ci ) {
            if(ci->_etraps > 0) {
                SQExceptionTrap &l_et = _etraps.top();
                ci->_ip = l_et._ip;
                _top = l_et._stacksize;
                _stackbase = l_et._stackbase;
                _stack._vals[_stackbase + l_et._extarget] = currerror;
...
void SQVM::CallDebugHook(SQInteger type,SQInteger forcedline)
{
    ...
    if(_debughook_native) {
        ...
        SQObjectPtr l_temp_reg;
        SQInteger nparams=5;
        Push(_roottable); Push(type); Push(func->_sourcename); Push(forcedline?forcedline:func->GetLine(ci->_ip)); Push(func->_name);
        Call(_debughook_closure,nparams,_top-nparams, l_temp_reg,SQFalse);
        Pop(nparams);
    }
    _debughook = true;
}
...
cloned_mt:
        ...
        if(_delegable(newobj)->_delegate && _delegable(newobj)->GetMetaMethod(this,MT_CLONED,closure)) {
            Push(newobj);
            Push(self);
            if(!CallMetaMethod(closure,MT_CLONED,2, l_temp_reg ))
                return false;
        }
        }
        target = newobj;
